<?php
/**
 *	DediStart WHMCS Server Provisioning version 1.0
 *
 *	@package     WHMCS
 *	@copyright   Andrezzz
...
function AndrezzzDediStart_API(array $params) {
    $url = 'https://www.dedistart.com/api/v1/';
    $data = [];
    $method = '';
    ...
    switch ($params['action']) {
        ...
        case 'Order':
            ...
            $disks = array();
            $addonid = AndrezzzDediStart_GetOption($params, 'addonid');

            for ($i = 1; $i < 5; $i++) {
                $diskid = AndrezzzDediStart_GetOption($params, 'disk' . $i . 'id');
            ...
            $data += array(
                'package' => AndrezzzDediStart_GetOption($params, 'packageid'),
                'ram' => AndrezzzDediStart_GetOption($params, 'ram'),
                'os' => AndrezzzDediStart_GetOption($params, 'osid'),
                'bandwidth' => AndrezzzDediStart_GetOption($params, 'bandwidthid'),
                'disk' => (count($disks) === 1) ? $disks[0] : $disks,
                'addon' => ($addonid !== 0) ? $addonid : '',
                'billing_term' => $billingCycles[$service->billingcycle] ?? 1,
                'ips' => AndrezzzDediStart_GetOption($params, 'ips'),
                'raid' => AndrezzzDediStart_GetOption($params, 'raid'),
                'notify_url' => Setting::getValue('SystemURL') . '/modules/servers/AndrezzzDediStart/callback.php',
            );
            break;
    ...
    curl_setopt($curl, CURLOPT_USERAGENT, 'Andrezzz - DediStart WHMCS');
...
function AndrezzzDediStart_Error($func, $params, Exception $err) {
    logModuleCall('Andrezzz - DediStart', $func, $params, $err->getMessage(), $err->getTraceAsString());
}

function AndrezzzDediStart_MetaData() {
    return array(
        'DisplayName' => 'Andrezzz - DediStart',
        'APIVersion' => '1.1',
        'RequiresServer' => true,
    );
}

function AndrezzzDediStart_ConfigOptions() {
    $os = array();
    $packages = array();
    $bandwidths = array();
    $disks = array();
    ...
    if (basename($_SERVER['SCRIPT_NAME'], '.php') === 'configproducts' && $_REQUEST['action'] === 'module-settings') {
        ...
        $packagesList = AndrezzzDediStart_API($params);
        ...
        $operatingSystems = AndrezzzDediStart_API($params);
        ...
        $bandwidthList = AndrezzzDediStart_API($params);
        ...
        $params['action'] = 'Disks';
        $disksList = AndrezzzDediStart_API($params);
        ...
        $addonsList = AndrezzzDediStart_API($params);
...
function AndrezzzDediStart_GetOption(array $params, $id, $default = NULL) {
    $options = AndrezzzDediStart_ConfigOptions();
...
function AndrezzzDediStart_TestConnection(array $params) {
    $err = '';

    try {
        $params['action'] = 'User Info';
        AndrezzzDediStart_API($params);
    } catch(Exception $e) {
        AndrezzzDediStart_Error(__FUNCTION__, $params, $e);
...
function AndrezzzDediStart_CreateAccount(array $params) {
    try {
        $params['action'] = 'Order';
        $order = AndrezzzDediStart_API($params);
...
function AndrezzzDediStart_SuspendAccount(array $params) {
    try {
...
function AndrezzzDediStart_UnsuspendAccount(array $params) {
    try {
        if ($params['callback']) return 'success';
...
function AndrezzzDediStart_TerminateAccount(array $params) {
    try {
        if (!$params['callback']) {
...
// function AndrezzzDediStart_ChangePassword(array $params) {
//     try {
//         AndrezzzDediStart_API($params);
//     } catch(Exception $err) {
//         AndrezzzDediStart_Error(__FUNCTION__, $params, $err);
...
// function AndrezzzDediStart_ChangePackage(array $params) {
//     try {
...
function AndrezzzDediStart_PowerUp(array $params) {
    try {
        AndrezzzDediStart_API($params);
    } catch(Exception $err) {
        AndrezzzDediStart_Error(__FUNCTION__, $params, $err);
...
function AndrezzzDediStart_PowerDown(array $params) {
    try {
        AndrezzzDediStart_API($params);
    } catch(Exception $err) {
        AndrezzzDediStart_Error(__FUNCTION__, $params, $err);
...
function AndrezzzDediStart_Reboot(array $params) {
    try {
        AndrezzzDediStart_API($params);
    } catch(Exception $err) {
        AndrezzzDediStart_Error(__FUNCTION__, $params, $err);
...
function AndrezzzDediStart_ClientAreaAPI(array $params) {
    try {
...
function AndrezzzDediStart_AdminCustomButtonArray() {
    return array(
        'Power Up' => 'PowerUp',
...
function AndrezzzDediStart_ClientAreaCustomButtonArray() {
    return array(
        'Power Up' => 'PowerUp',
        'Power Down' => 'PowerDown',
        'Reboot'=> 'Reboot',
...
function AndrezzzDediStart_ClientAreaAllowedFunctions() {
    return array('ClientAreaAPI');
}

function AndrezzzDediStart_ClientArea(array $params) {
    if ($params['moduletype'] !== 'AndrezzzDediStart') return;

    try {
        if (strpos($params['domain'], 'ORDER-') !== false) throw new Exception('The order is pending.', 1);

        $params['action'] = 'Server';
        $server = AndrezzzDediStart_API($params);
        ...
        $currentGraph = AndrezzzDediStart_API($params);
        
        $params['interval'] = 'last_month';
        $lastMonthGraph = AndrezzzDediStart_API($params);
...
function AndrezzzDediStart_AdminLink(array $params) {
    try {
        $params['action'] = 'User Info';
        $account = AndrezzzDediStart_API($params);
